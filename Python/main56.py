#######################################################################################################
#   Problem Statement :   Design automation script which accept directory name and write names Design
#   automation that directory into log directory of duplicate files from named as Log.txt. Log.txt
#   file should be created into current
#######################################################################################################
import os
import sys
import time
...
def Delete_File(Dic1):

    Results = list(filter(lambda x: len(x)) > 1, Dic1.values())
...
def Find_Duplicate(HashFile, Path):
